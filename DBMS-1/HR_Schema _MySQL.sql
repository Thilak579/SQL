...
#display the id and name of employee who works is dept 10
...
#to display the names and dept ids of employees whose salary is less that 5000$

select first_name, last_name, department_id
from employees
where salary<5000;

#display the details of employees who draw a salary between 5000 and 10000
select *
from employees
where salary between 5000 and 10000;

#display the id, firstname,lastname of the employees who were hired in the period of 1990 and 1995
select employee_id, first_name, last_name
from employees
where hire_date between '1990-01-01' and '1995-12-31';

#to display the names of th employees who have salary not equal to 6000
select first_name,last_name
from employees
where salary!=6000;

#display the details of the employees whose name has letter 'x' in it
select first_name
from employees
where first_name like '%x%';

#display the names of the employees whose last name ends with 'a'.
select first_name, last_name
from employees
where last_name like '%a';

#display the details of the employeees wjose name begins with 'h' and ends with 'n'
select *
from employees
where first_name like 'h%r%n';

#display the employees who are noty assisgned to any dept
select first_name,last_name
from employees
where department_id is null;

#display the employees who are working in a dept
select first_name,last_name
from employees
where department_id is not null;

#Display the ids of the departments on the company
...
select truncate(34.675,1);

select truncate(23.3223,1);
...
select first_name, char_length(first_name), length(first_name)
from employees;

select instr('Hello','e');

select substring('hello how are you',2,8);

select reverse('how');

select replace('hello how are you','o','xx');
...
select datediff('2021-02-16','2021-02-01');

select date_format('2021-02-16','%d-%m-%y');

select str_to_date('01-02-2021','%d-%m-%Y');

select convert(10,char(2));
...
#calculate the count of employees in the department
...
#A company is analysing the salaried drawn by different degignations.
#generate a report which describes the extremes of the salary and the mean  value of it

select job_id,min(salary),max(salary),avg(salary)
from employees
group by job_id;

#A company is analysing the salaried drawn by different departments and degignations.
#generate a report which describes the extremes of the salary and the mean  value of it
...
#write a query to get the count of employees in every department. display the
#departments where more than 10 employees are working
...
#find the departments where the max salary is greater than 10000
...
#display the employee id employee name and job_id and job_title
 select e.employee_id, e.first_name, e.last_name, e.job_id, j.job_title
 from employees e
 join jobs j
 on e.job_id=j.job_id;
 
 #display the employee_id, names, department_id, department_names
 #and the city where they belong to
 
 select e.employee_id, e.first_name, e.last_name, e.department_id, d.department_name, l.city
 from employees e
 join departments d
 on e.department_id=d.department_id
 join locations l
 ...
 #display the name of the cities and the copuntries they belong to
  select l.city,c.country_name
  from locations l
  join countries c
  on l.country_id=c.country_id;
  
  #display the list of employee names and countries the belog to
 ...
 # display the list of employees and the region they belong to
...
#display the names of the employees and departments who work in seattle
 ...
 #display the details of the employees who work as a manager in any department
 ...
 #The VP of the company needs the list of reps working in the dept of sales. generate a list
 ...
 #The VP needs the names of the departments and the budget needed to be allocated
 # foreach department to pay the salaries of all employees working in them.alter
 #generate the report and sort the departments in an order from highest to lowest
...
drop table bank_branch_pl;

#Conditional constructs

#Display the details of the employees along with a tagging based on salary
#Tagging to be done as:
#if salaryt is less than 10000$ then 'low paid'
#if salary is greater than = 10000$ then 'high paid'

select * , if(salary<10000,'low paid','high paid') as salary
from employees;

select * , if(salary<10000,salary*0.1,salary*0.05) as Bonus
from employees;

#Display the details of the employees along with a tagging based on salary
#Tagging to be done as:
#if salaryt is less than 10000$ then 'low paid'
# if salary is between 10000 and 15000 then 'moderately paid'
#if salary is greater than = 15000$ then 'high paid'

select *,
case when salary<10000 then 'low paid'
when salary between 10000 and 15000 then 'moderately paid'
else 'high paid'
end as salary_tag
from employees;
...
#display the job history table also include firstname and lastname of the employees
...
#display the details of the employees and dept names and also include the names of the employees who are not working in any dept.
...
#display the employee id ,names of the employees and their manager_id
...
#display the e_id,e_name for all employees who earn more than the avg salary of an org
select employee_id, first_name, last_name
from employees
where salary>(select avg(salary)
from employees);

#display the emp names,id, salary of all emp who report to payam
...
#display the emp names and hiredate for all emps in the same dept as clara
...
#Display the info of all employees who dos not work in these depts whose manager id with in range 100 and 200
...
#display the department details whicjh has employees
...
#disply the name & dept_id od all depts that has atleast one employee with salary greatrer rgan 10000 using in & exists
...
#display dept wise, jobwise, count with the sub total
...
#Display the employee_id, name , salary,department name and city of an employee
#--who is earning the highest salary among the employees who joined in the year 1997
...
#display the dept_name along with employee count

select min(e.cnt), max(e.cnt), avg(e.cnt)
from (select department_id,count(employee_id) cnt
from employees
group by department_id) as e
join departments d
on e.department_id=d.department_id;

#display th emplloyee details whose salary is equal to the minimun salary of their respective dept
