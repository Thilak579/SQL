...
select bad.Account_Number, sum(Transaction_amount) as Fr_t  
...
case when bat.Transaction_Date = bh.Holiday then sum(Transaction_amount) else null 
end as 'H_agg'
from bank_account_details bad 
join bank_account_transaction bat 
on bad.Account_Number = bat.Account_Number 
cross join bank_holidays bh 
on bat.Transaction_Date = bh.Holiday 
where bad.Account_type = 'Credit card'
union
select bad.Account_type, bad.Account_Number, 
case when bat.Transaction_Date != bh.Holiday then sum(Transaction_amount) else null 
end as 'NH_agg'
...
select bat.Transaction_Date, 'Happy Holiday' as Message, bh.Holiday 
from bank_account_transaction bat 
cross join bank_holidays bh 
on bat.Transaction_Date =  bh.Holiday 
where month(Transaction_Date) = 3 and bat.Transaction_Date = bh.Holiday;
...
select b.Account_Number, c.Transaction_Date, sum(c.Transaction_amount * 0.07) as interest 
from bank_account_relationship_details a 
join bank_account_relationship_details b 
on a.Account_Number= b.Linking_Account_Number and a.Account_type <> b.Account_type and b.Account_type like '%recurr%' 
join bank_account_transaction c
on b.Account_Number = c.Account_Number and (c.Transcation_channel like '%ecs%' or c.Transcation_channel like '%deposit%') 
...
from bank_customer_export natural join bank_customer;
...
select be.*
from bank_customer_export be natural join bank_customer bc;
