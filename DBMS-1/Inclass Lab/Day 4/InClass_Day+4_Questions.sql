...
select bc.customer_id, bc.customer_name, avg(bad.balance_amount)
...
case when account_type like '%credit%' then IF(balance_amount=0,transaction_amount,balance_amount)
     else balance_amount
end as balance
...
#or
select customer_id, bad.account_number,
IF(balance_amount=0,transaction_amount,balance_amount)
from bank_account_details bad
join bank_account_transaction bat
on bad.account_number=bat.account_number
...
select bc.customer_id, bad.account_number, bad.balance_amount, bat.transaction_amount, bat.transaction_date
from bank_customer bc
join bank_account_details bad
on bc.customer_id=bad.customer_id
join bank_account_transaction bat
...
join bank_account_transaction bat
on bad.account_number=bat.account_number
where quarter(transaction_date)=1; 
...
select bad.customer_id, bad.account_number, bad.account_type,
if(transaction_amount<0,balance_amount+transaction_amount,balance_amount) as balance
from bank_account_details bad
join bank_account_transaction bat
on bad.account_number=bat.account_number
...
select bad.*,bard.account_number,bard.account_type
from bank_account_details bad
...
select bad.*,bard.account_number,bard.account_type,bat.transaction_amount
from bank_account_details bad
join bank_account_relationship_details bard
on bad.account_number=bard.linking_account_number
join bank_account_transaction bat
...
select bard.account_number as cc_no,bad.account_number as primary_account_no,bad.account_type,sum(bat.transaction_amount),bard.account_type
from bank_account_details bad
join bank_account_relationship_details bard
on bard.linking_account_number = bad.account_number
join bank_account_transaction bat
on bat.account_number= bard.account_number
where bard.account_type='Credit card'
group by bard.account_number,bad.account_number, bad.account_type;
...
join bank_account_transaction bat
on bat.account_number= bard.account_number
where bard.account_type like '%Credit%'
group by bard.account_number,bad.account_number, bad.account_type; 
...
select bad.account_number as primary_Acc, bard.account_number as rd_no ,sum(transaction_amount)
from bank_account_details bad
join  bank_account_Relationship_details bard
on  bad.account_number= bard.linking_Account_number
join bank_Account_Transaction bat 
...
join bank_Account_Transaction bat 
on bard.account_number = bat.account_number
where year(Transaction_date)=2020 and month(Transaction_date)=2
group by bad.account_number , bard.account_number;
...
-- sum of Jan month transaction amount , 
 ...
 select bat1.account_number,sum(bat1.Transaction_amount) as jan_amt , sum(bat2.transaction_Amount) as feb_amt
...
select bc.customer_name,bad1.account_number
from Bank_customer bc
join  bank_Account_details bad1 
on bc.customer_id= bad1.customer_id
join  bank_Account_details bad2 
on bc.customer_id= bad2.customer_id
join  bank_Account_details bad3 
on bc.customer_id= bad3.customer_id
where bad1.account_type='SAVINGS' and bad2.account_type= 'RECURRING DEPOSITS' and bad3.account_type ='Credit Card';

# Question 18:
# 18) Display savings accounts and its corresponding Recurring deposits transactions that are occuring more than 4 times.

select bad.account_number as primary_Acc, bard.account_number as rd_no ,count(*)
from bank_account_details bad
join  bank_account_Relationship_details bard
on  bad.account_number= bard.linking_Account_number
join bank_Account_Transaction bat 
...
select bad.account_number as primary_Acc, bard.account_number as rd_no ,count(distinct month(transaction_Date))
from bank_account_details bad
join  bank_account_Relationship_details bard
on  bad.account_number= bard.linking_Account_number
join bank_Account_Transaction bat 
...
select bad.account_number , count(*)
from bank_account_details bad
join bank_Account_Transaction bat
on bad.account_number = bat.account_number
