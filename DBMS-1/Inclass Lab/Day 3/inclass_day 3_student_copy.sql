...
select product, max(purchase_cost)-min(purchase_cost) as difference
from bank_inventory_pricing
group by product;
...
from bank_inventory_pricing
where quantity>4 and purchase_cost is not null
group by product;
...
select product, avg(estimated_sale_price)+avg(estimated_sale_price)*15/100
from bank_inventory_pricing
group by product
...
select product, month, avg(price)
from bank_inventory_pricing
