...
select e.employee_id, e.first_name, e.last_name, e.salary,e.job_id, d.department_id
...
Start transaction;
select * from bank_customer where customer_id > 1230008 for update;
...
select * 
from bank_account
where customer_id=123001 for update;
update bank_account
set balance_amount = balance_amount + 0.03*balance_amount
...
where account_number ='4000-1956-3401' for update;
update bank_account set balance_amount = balance_amount + 0.03 * balance_amount 
where customer_id = 123001;
commit;
#After transaction of session-1 is complete with commit, below two transaction can be done at a time
# session-2 insert intention lock 1
start transaction;
insert into bank_account (account_number, balance_amount)
values('4000-1956-3401');
